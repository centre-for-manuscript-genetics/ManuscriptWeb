<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:eg="http://www.tei-c.org/ns/Examples" xmlns:t="http://www.tei-c.org/ns/1.0" exclude-result-prefixes="t eg" version="1.0">
  ...
  <xsl:template match="/">
    ...
    <html>
      <head>
        ...
        <script type="text/javascript" src="{$CETEI}"/>
  ...
  <xsl:template match="*[namespace-uri(.) = 'http://www.tei-c.org/ns/1.0']">
    <xsl:element name="tei-{translate(local-name(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')}">
      <xsl:if test="namespace-uri(parent::*) != namespace-uri(.)">
      ...
      <xsl:attribute name="data-teiname">
  ...
  <xsl:template name="elements">
    <xsl:for-each select="//*[namespace-uri(.) = 'http://www.tei-c.org/ns/1.0']">
      <xsl:if test="not(preceding::*[local-name() = local-name(current())])">"<xsl:value-of select="local-name()"/>"<xsl:if test="position() != last()">,</xsl:if>
            </xsl:if>
        </xsl:for-each>    
  </xsl:template>
...
<xsl:call-template name="rewrite-selectors">
                    <xsl:with-param name="in" select="@selector"/>
                </xsl:call-template> {
  <xsl:value-of select="."/>       
}
      </xsl:when>
  ...
  <xsl:template name="normalize-selectors">
    ...
    <xsl:call-template name="normalize-child-selector">
      <xsl:with-param name="selector">
                <xsl:call-template name="normalize-list-selector">
                    <xsl:with-param name="selector" select="$in"/>
                </xsl:call-template>
            </xsl:with-param>
    </xsl:call-template>    
  </xsl:template>
  ...
  <xsl:template name="normalize-child-selector">
    ...
    <xsl:choose>
      <xsl:when test="contains($selector, ' &gt; ')">
        <xsl:call-template name="normalize-child-selector">
          <xsl:with-param name="selector">
                        <xsl:value-of select="substring-before($selector,' &gt; ')"/>&gt;<xsl:value-of select="substring-after($selector,' &gt; ')"/>
                    </xsl:with-param>
        </xsl:call-template>
      </xsl:when>
      <xsl:when test="contains($selector, ' &gt;')">
  ...
  <xsl:template name="rewrite-child-selector">
    ...
    <xsl:choose>
      <xsl:when test="contains($selector, '&gt;')">
        <xsl:call-template name="normalize-child-selector">
          <xsl:with-param name="result">&gt;tei-<xsl:value-of select="substring-after($selector,'&gt;')"/>
                    </xsl:with-param>
          <xsl:with-param name="selector">
      ...
      <xsl:otherwise>
<xsl:if test="not(starts-with($result,'tei-') or starts-with($selector, 'tei-'))">tei-</xsl:if>
...
<xsl:if test="not(starts-with($result,'tei-') or starts-with($selector, 'tei-'))">tei-</xsl:if>
  ...
  <xsl:template name="rewrite-selectors">
    ...
    <xsl:variable name="reg">
            <xsl:call-template name="normalize-selectors">
                <xsl:with-param name="in" select="$in"/>
            </xsl:call-template>
        </xsl:variable>
<xsl:call-template name="normalize-list-selector">
            <xsl:with-param name="selector">
                <xsl:call-template name="rewrite-descendent-selector">
                    <xsl:with-param name="selector">
                        <xsl:call-template name="rewrite-child-selector">
                            <xsl:with-param name="selector" select="$reg"/>
                        </xsl:call-template>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:with-param>
